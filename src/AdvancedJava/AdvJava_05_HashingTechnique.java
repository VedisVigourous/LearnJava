...
public class AdvJava_05_HashingTechnique {
    ...
    /* ~ Internal Working -
       * used a Fn = f(x) = x % (number of Indices)
       ...
       Example:
        ...
        1. f(29) = 29 % 10 = 9 (9th index allotted)
        2. f(58) = 58 % 10 = 8 (8th index allotted)
        2. f(25) = 25 % 10 = 5 (5th index allotted)
        2. f(13) = 13 % 10 = 3 (3rd index allotted)

        * i.e. this technique is very fast in index allotting

        *** Known Problems ***
        1. HashCollision - When two numbers are to be allotted on a same index ex. 11 & 21 both will get 1st index
           1.1 Solution 1 [Open Addressing - Increasing indexes i.e. Empty spaces are kept in HashTable so that the other number be allotted to nearest empty index.
